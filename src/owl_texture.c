				      ...
				      VkImageLayout layout)
{
	...
	vkCmdPipelineBarrier(command_buffer, src_stage, dst_stage, 0, 0, NULL,
			     0, NULL, 1, &barrier);
			    ...
			    struct owl_texture *texture)
{
	...
	if (OWL_TEXTURE_SOURCE_DATA == desc->source) {
		...
		if (OWL_TEXTURE_TYPE_2D == desc->type ||
			...
			data = stbi_load(desc->path, &width, &height, &channels,
					 STBI_rgb_alpha);
			if (!data) {
				ret = OWL_ERROR_FATAL;
				goto error_free_upload_data;
			...
			upload_data = owl_renderer_upload_allocate(
				r, bitmap_size, &upload_allocation);
			if (!upload_data) {
			...
			for (i = 0; i < (int32_t)OWL_ARRAY_SIZE(names); ++i) {
				...
				OWL_SNPRINTF(path, OWL_TEXTURE_MAX_PATH_LENGTH,
					     "%s/%s", desc->path, names[i]);

				/* load the image */
				data = stbi_load(path, &width, &height,
                 ...
                 */
				OWL_MEMCPY(upload_data + offset, data,
					   bitmap_size);
	...
	if (OWL_TEXTURE_TYPE_COMPUTE == desc->type)
		...
		texture->mipmaps = owl_texture_calculate_mipmaps(
...

	{
		...
		vk_result =
			vkCreateImage(r->device, &info, NULL, &texture->image);
		if (vk_result) {
			ret = OWL_ERROR_FATAL;
			goto error_free_upload_data;
		}
...

	{
		...
		vkGetImageMemoryRequirements(r->device, texture->image,
					     &requirements);
		...
		vk_result = vkAllocateMemory(r->device, &info, NULL,
					     &texture->memory);
		if (vk_result) {
			ret = OWL_ERROR_NO_MEMORY;
	...
	if (OWL_TEXTURE_TYPE_CUBE == desc->type) {
		...
		vkCmdCopyBufferToImage(r->im_command_buffer,
...

	{
		...
		vkUpdateDescriptorSets(r->device, OWL_ARRAY_SIZE(writes),
				       writes, 0, NULL);
	}
