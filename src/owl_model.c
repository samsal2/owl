...
static int owl_model_load_images(struct owl_renderer *r,
				 struct cgltf_data *gltf, struct owl_model *m)
{
	int32_t i;
	int ret = OWL_OK;
...
static void owl_model_unload_images(struct owl_renderer *r, struct owl_model *m)
{
	int32_t i;
	...
	for (i = 0; i < m->num_materials; ++i) {
		...
		if (in_material->has_pbr_metallic_roughness) {
			...
			if (texture) {
				...
				out_material->base_color_texcoord =
					view->texcoord;
			} else {
				out_material->base_color_texture = -1;
				out_material->base_color_texcoord = -1;
			...
			if (texture) {
				out_material->metallic_roughness_texture =
					(int32_t)(texture - first);
				out_material->metallic_roughness_texcoord =
					view->texcoord;
			} else {
				out_material->metallic_roughness_texture = -1;
				out_material->metallic_roughness_texcoord = -1;
...

		{
			...
			if (texture) {
				out_material->emissive_texture =
					(int32_t)(texture - first);
				out_material->emissive_texcoord =
					view->texcoord;
			} else {
			...
			if (texture) {
				out_material->occlusion_texture =
					(int32_t)(texture - first);
				out_material->occlusion_texcoord =
					view->texcoord;
			} else {
			...
			out_material->emissive_factor[0] =
				in_material->emissive_factor[0];
			out_material->emissive_factor[1] =
				in_material->emissive_factor[1];
			out_material->emissive_factor[2] =
				in_material->emissive_factor[2];
			out_material->emissive_factor[3] = 1.0F;
		}
		...
		if (in_material->has_pbr_specular_glossiness) {
			...
			if (texture) {
				out_material->specular_glossiness_texture =
					(int32_t)(texture - first);
				out_material->specular_glossiness_texcoord =
					view->texcoord;
			} else {
			...
			texture = view->texture;
			if (texture)
				out_material->diffuse_texture =
					(int32_t)(texture - first);
			else
				out_material->diffuse_texture = -1;

			out_material->diffuse_factor[0] =
				specular->diffuse_factor[0];
			out_material->diffuse_factor[1] =
				specular->diffuse_factor[1];
			out_material->diffuse_factor[2] =
				specular->diffuse_factor[2];
			out_material->diffuse_factor[3] =
				specular->diffuse_factor[3];

			out_material->specular_factor[0] =
				specular->specular_factor[0];
			out_material->specular_factor[1] =
				specular->specular_factor[1];
			out_material->specular_factor[2] =
				specular->specular_factor[2];
...

		{
				...
				if (-1 != out_material->diffuse_texture) {
					...
					texture =
						&m->textures
							 [out_material
								  ->diffuse_texture];
					image = &m->images[m->textures[texture->image]
								   .image];
			...
			vkUpdateDescriptorSets(r->device,
					       OWL_ARRAY_SIZE(writes), writes,
					       0, NULL);
		}
	}
...
static void owl_model_deinit_all_primitives(struct owl_model_all_primitives *p)
{
	OWL_FREE(p->indices);
	OWL_FREE(p->vertices);
}
				...
				struct owl_model *m)
{
	...
	for (i = 0; i < m->num_nodes; ++i) {
		...
		if (in_node->parent)
			out_node->parent =
				(int32_t)(in_node->parent - gltf->nodes);
		else
			out_node->parent = -1;

		OWL_ASSERT(in_node->children_count <
			   OWL_ARRAY_SIZE(out_node->children));
		...
		if (in_node->mesh) {
			...
			out_mesh->num_primitives =
				(int32_t)in_mesh->primitives_count;
			for (j = 0; j < out_mesh->num_primitives; ++j) {
				...
				struct cgltf_attribute const *attr = NULL;
				struct owl_model_primitive *out_primitive =
					NULL;
				struct cgltf_primitive const *in_primitive =
					NULL;

				in_primitive = &in_mesh->primitives[j];
				out_mesh->primitives[j] = m->num_primitives;
				out_primitive =
					&m->primitives[m->num_primitives++];

				attr = owl_find_gltf_attribute(in_primitive,
							       "POSITION");
				if (attr) {
					position = owl_resolve_gltf_accessor(
						attr->data);
					position_stride = owl_model_gltf_stride(
						attr->data->type);
					num_local_vertices =
						(int32_t)attr->data->count;
				...
				attr = owl_find_gltf_attribute(in_primitive,
							       "NORMAL");
				if (attr) {
					normal = owl_resolve_gltf_accessor(
						attr->data);
					normal_stride = owl_model_gltf_stride(
						attr->data->type);
				}
				...
				attr = owl_find_gltf_attribute(in_primitive,
							       "TEXCOORD_0");
				if (attr) {
					uv0 = owl_resolve_gltf_accessor(
						attr->data);
					uv0_stride = owl_model_gltf_stride(
						attr->data->type);
				}
				...
				attr = owl_find_gltf_attribute(in_primitive,
							       "TEXCOORD_1");
				if (attr) {
					uv1 = owl_resolve_gltf_accessor(
						attr->data);
					uv1_stride = owl_model_gltf_stride(
						attr->data->type);
				}
				...
				attr = owl_find_gltf_attribute(in_primitive,
							       "JOINTS_0");
				if (attr) {
					joints0 = owl_resolve_gltf_accessor(
						attr->data);
					joints0_stride = owl_model_gltf_stride(
						attr->data->type);
					if (attr->data->component_type ==
				...
				if (attr) {
					weights0 = owl_resolve_gltf_accessor(
						attr->data);
					weights0_stride = owl_model_gltf_stride(
						attr->data->type);
				}
				...
				attr = owl_find_gltf_attribute(in_primitive,
							       "COLOR_0");
				if (attr) {
					color0 = owl_resolve_gltf_accessor(
						attr->data);
					color0_stride = owl_model_gltf_stride(
						attr->data->type);
				}
            ...
            OWL_ASSERT(vertex->uv0[1] < 1.0F && vertex->uv0[1] > 0.0F);
				...
				if (in_primitive->indices) {
					struct cgltf_accessor *accessor =
						in_primitive->indices;

					num_local_indices =
						(int32_t)accessor->count;

					OWL_ASSERT(num_indices +
							   num_local_indices <=
						   p->num_indices);
					...
					case cgltf_component_type_r_32u: {
						int32_t l;
						uint32_t const *data =
							owl_resolve_gltf_accessor(
								accessor);
						for (l = 0;
						     l < num_local_indices; ++l)
							p->indices[num_indices +
								   l] =
								data[l] +
								num_vertices;
					} break;
...

				{
					...
					out_primitive->num_indices =
						num_local_indices;
					out_primitive->num_vertices =
						num_local_vertices;
					out_primitive->has_indices =
						!!num_local_indices;
			...
			for (j = 0;
			     j < (int32_t)OWL_ARRAY_SIZE(out_mesh->ssbos);
			     ++j) {
...

			{
				...
				vkGetBufferMemoryRequirements(
					device, out_mesh->ssbos[0],
					&requirements);

				aligned_size =
					OWL_ALIGN_UP_2(requirements.size,
				...
				for (j = 0; j < (int32_t)OWL_ARRAY_SIZE(
							out_mesh->ssbos);
				     ++j) {
					vk_result = vkBindBufferMemory(
						device, out_mesh->ssbos[j],
						out_mesh->ssbo_memory,
						j * aligned_size);
					OWL_ASSERT(!vk_result);
				}

				{
					...
					vk_result = vkMapMemory(
						device, out_mesh->ssbo_memory,
						0, VK_WHOLE_SIZE, 0, &data);
					OWL_ASSERT(!vk_result);
					for (j = 0;
					     j < (int32_t)OWL_ARRAY_SIZE(
							 out_mesh->ssbos);
					     ++j) {
				...
				for (j = 0; j < (int32_t)OWL_ARRAY_SIZE(
					...
					for (l = 0; l < (int32_t)OWL_ARRAY_SIZE(
								ssbo->joints);
					     ++l)
						OWL_M4_IDENTITY(
							ssbo->joints[l]);
			...
			{
				VkDescriptorSetLayout layouts[OWL_ARRAY_SIZE(
					out_mesh->ssbos)];
				VkDescriptorSetAllocateInfo info;
				VkResult vk_result;
				OWL_UNUSED(vk_result);
				...
				for (j = 0;
				     j < (int32_t)OWL_ARRAY_SIZE(layouts); ++j)
					layouts[j] =
						r->model_storage_descriptor_set_layout;
			...
			}
			{
				VkDescriptorBufferInfo
					descriptors[OWL_ARRAY_SIZE(
						out_mesh->ssbos)];
				VkWriteDescriptorSet
					writes[OWL_ARRAY_SIZE(out_mesh->ssbos)];

				for (j = 0;
				     j < (int32_t)OWL_ARRAY_SIZE(descriptors);
				     ++j) {
					descriptors[j].buffer =
						out_mesh->ssbos[j];
					descriptors[j].offset = 0;
					descriptors[j].range = sizeof(
					...
					writes[j].pNext = NULL;
					writes[j].dstSet =
						out_mesh->ssbo_descriptor_sets[j];
...
static void owl_model_unload_nodes(struct owl_renderer *r, struct owl_model *m)
{
	int32_t i;
	VkDevice const device = r->device;
				  ...
				  struct owl_model *m)
{
...

	{
		...
		vk_result =
			vkCreateBuffer(device, &info, NULL, &m->vertex_buffer);
		OWL_ASSERT(!vk_result);
	}

	{
		...
		vkGetBufferMemoryRequirements(device, m->vertex_buffer,
					      &requirements);
	...
	if (p->num_indices) {
...

		{
			...
			vkGetBufferMemoryRequirements(device, m->index_buffer,
						      &requirements);
...

		{
			...
			data = owl_renderer_upload_allocate(r, size,
							    &allocation);
			OWL_ASSERT(data);
				...
				struct owl_model *m)
{
	...
	for (i = 0; i < m->num_skins; ++i) {
		...
		if (in_skin->skeleton)
			out_skin->root =
				(int32_t)(in_skin->skeleton - gltf->nodes);
		else
			out_skin->root = -1;

		OWL_ASSERT(in_skin->joints_count <
		...
		if (in_skin->inverse_bind_matrices) {
			...
			for (j = 0; j < out_skin->num_inverse_bind_matrices;
			     ++j)
				OWL_M4_COPY(matrices[j],
					    out_skin->inverse_bind_matrices[j]);
		}
...
static void owl_model_unload_skins(struct owl_renderer *r, struct owl_model *m)
{
	OWL_UNUSED(r);
	OWL_UNUSED(m);
}
				     ...
				     struct owl_model *m)
{
	...
	for (i = 0; i < m->num_animations; ++i) {
		...
		struct cgltf_animation const *in_animation =
			&gltf->animations[i];
		...
		if (in_animation->name) {
			int32_t max_length =
				OWL_ARRAY_SIZE(out_animation->name);
			OWL_STRNCPY(out_animation->name, in_animation->name,
				    max_length);
		} else {
			int32_t max_length =
				OWL_ARRAY_SIZE(out_animation->name);
		...
		out_animation->num_samplers =
			(int32_t)in_animation->samplers_count;
		for (j = 0; j < out_animation->num_samplers; ++j) {
			...
			out_sampler->num_inputs =
				(int32_t)in_sampler->input->count;
			...
			out_sampler->num_outputs =
				(int32_t)in_sampler->output->count;
			switch (in_sampler->output->type) {
			case cgltf_type_vec3: {
				owl_v3 const *outputs;
				outputs = owl_resolve_gltf_accessor(
					in_sampler->output);

				for (k = 0; k < out_sampler->num_outputs; ++k) {
					OWL_V4_ZERO(out_sampler->outputs[k]);
					OWL_V3_COPY(outputs[k],
						    out_sampler->outputs[k]);
				}
			} break;
		...
		for (j = 0; j < out_animation->num_channels; ++j) {
			...
			out_channel->node = (int32_t)(in_channel->target_node -
						      gltf->nodes);

			id = (int32_t)(in_channel->sampler -
				       in_animation->samplers);
...
static void owl_model_unload_roots(struct owl_renderer *r, struct owl_model *m)
{
	OWL_UNUSED(r);
	OWL_UNUSED(m);
}
...
static void owl_model_resolve_node_matrix(struct owl_model const *m, int32_t id,
					  owl_m4 matrix)
{
	int32_t p;
...
OWLAPI int owl_model_update_animation(struct owl_renderer *r,
				      struct owl_model *m, float dt, int32_t id)
{
	int32_t i;
	...
	for (i = 0; i < animation->num_channels; ++i) {
		...
		for (j = 0; j < sampler->num_inputs - 1; ++j) {
			...
			case OWL_ANIMATION_PATH_TRANSLATION: {
				owl_v3_mix(sampler->outputs[j],
					   sampler->outputs[j + 1], a,
					   node->translation);
			} break;

			case OWL_ANIMATION_PATH_ROTATION: {
				owl_v4_quat_slerp(sampler->outputs[j],
						  sampler->outputs[j + 1], a,
						  node->rotation);
				owl_v4_normalize(node->rotation,
						 node->rotation);
			} break;
