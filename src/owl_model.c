                                    ...
                                    struct owl_model *m)
{
    ...
    for (i = 0; i < m->num_materials; ++i) {
...

        {
            ...
            vkUpdateDescriptorSets(r->device, OWL_ARRAY_SIZE(writes), writes,
                                   0, NULL);
        }
    }
      ...
      out_node->mesh = (int32_t)(in_mesh - gltf->meshes);
            ...
            OWL_ASSERT(vertex->uv0[1] < 1.0F && vertex->uv0[1] > 0.0F);
                ...
                if (in_primitive->indices) {
                    ...
                    OWL_ASSERT(num_indices + num_local_indices <=
                               p->num_indices);
...

            {
                ...
                for (j = 0; j < (int32_t)OWL_ARRAY_SIZE(out_mesh->ssbos);
                     ++j) {
    ...
    for (i = 0; i < animation->num_channels; ++i) {
        ...
        for (j = 0; j < sampler->num_inputs - 1; ++j) {
            ...
            case OWL_ANIMATION_PATH_ROTATION: {
                owl_v4_quat_slerp(sampler->outputs[j], sampler->outputs[j + 1],
                                  a, node->rotation);
