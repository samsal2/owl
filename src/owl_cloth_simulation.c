				     ...
				     int32_t height, char const *material)
{
	...
	sim->particles =
		OWL_MALLOC(sim->num_particles * sizeof(*sim->particles));
	if (!sim->particles) {
	...
	for (i = 0; i < height; ++i) {
		for (j = 0; j < width; ++j) {
			...
			if (0 < j) {
				...
				particle->distances[0] = owl_v3_distance(
					particle->position, link->position);
			} else {
			...
			if (0 < i) {
				...
				particle->distances[1] = owl_v3_distance(
					particle->position, link->position);
			} else {
			...
			if ((width - 1) > j) {
				...
				particle->distances[2] = owl_v3_distance(
					particle->position, link->position);
			} else {
			...
			if ((height - 1) > i) {
				...
				particle->distances[3] = owl_v3_distance(
					particle->position, link->position);
			} else {
					...
					float dt)
{
	...
	for (i = 0; i < sim->num_particles; ++i) {
		...
		if (particle->movable) {
			...
			OWL_V3_SUB(particle->position,
				   particle->previous_position,
				   particle->velocity);

			/* velocity *= 1.0F - OWL_DAMPING */
			OWL_V3_SCALE(particle->velocity, 1.0F - OWL_DAMPING,
				     particle->velocity);

			/* delta_position = velocity + acceleration * dt */
			OWL_V3_SCALE(particle->acceleration, dt,
				     delta_position);
			OWL_V3_ADD(delta_position, particle->velocity,
				   delta_position);
			...
			for (j = 0; j < OWL_STEPS; ++j) {
				...
				for (side = 0; side < 4; ++side) {
					...
					if (link) {
						...
						OWL_V3_SUB(link->position,
							   particle->position,
							   delta);

						/* calculate the contraints factor */
						factor =
							1 - (particle->distances
								     [side] /
							     owl_v3_magnitude(
								     delta));

						/* find the correction value */
						OWL_V3_SCALE(delta, factor,
							     correction);

						/* half the correction value and apply it to the
                         * particle */
						OWL_V3_SCALE(correction, 0.5F,
							     correction);
						OWL_V3_ADD(particle->position,
							   correction,
							   particle->position);
