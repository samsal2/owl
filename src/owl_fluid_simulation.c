...
OWLAPI int owl_fluid_simulation_init(struct owl_fluid_simulation *sim,
...

  {
    ...
    vk_result =
        vkBindBufferMemory(device, sim->uniform_buffer, sim->uniform_memory, 0);
    if (vk_result) {
      ret = OWL_ERROR_FATAL;
    ...
    vk_result =
        vkMapMemory(device, sim->uniform_memory, 0, VK_WHOLE_SIZE, 0, &data);
    if (vk_result) {
      ret = OWL_ERROR_FATAL;
      goto error_free_uniform_memory;
    }
...

  {
    ...
    vk_result =
        vkAllocateDescriptorSets(device, &info, &sim->uniform_descriptor_set);
    if (vk_result)
      goto error_free_uniform_memory;
  }
