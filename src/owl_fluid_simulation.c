				 ...
				 struct owl_fluid_simulation_buffer *buffer)
{
...

	{
		...
		vk_result = vkAllocateDescriptorSets(
			device, &info, &buffer->storage_descriptor_set);
		if (vk_result) {
			ret = OWL_ERROR_FATAL;
...
OWLAPI void
owl_fluid_simulation_buffer_deinit(struct owl_renderer *r,
				   struct owl_fluid_simulation_buffer *buffer)
{
	VkDevice const device = r->device;
				     ...
				     struct owl_renderer *r)
{
...

	{
		...
		vkGetBufferMemoryRequirements(device, sim->uniform_buffer,
					      &requirements);
		...
		vk_result = vkMapMemory(device, sim->uniform_memory, 0,
...

	{
		...
		vk_result = vkAllocateDescriptorSets(
			device, &info, &sim->uniform_descriptor_set);
		if (vk_result)
			goto error_free_uniform_memory;
	}
	...
	for (i = 0; i < 2; ++i) {
		...
		ret = owl_fluid_simulation_buffer_init(
			r, vec, OWL_SIM_RESOLUTION, OWL_SIM_RESOLUTION, buffer);
		if (ret)
	...
	for (i = 0; i < 2; ++i) {
		...
		ret = owl_fluid_simulation_buffer_init(
			r, dye, OWL_DYE_RESOLUTION, OWL_DYE_RESOLUTION, buffer);
		if (ret)
			if (ret)
...
error_free_command_buffer:
	vkFreeCommandBuffers(device, sim->command_pool, 1,
			     &sim->command_buffer);
					...
					struct owl_renderer *r, float dt)
{
	...
	if (sim->requires_record) {
		...
		sim->uniform->sim_texel_size[0] =
			1.0F / (float)OWL_SIM_RESOLUTION;
		sim->uniform->sim_texel_size[1] =
			1.0F / (float)OWL_SIM_RESOLUTION;
		sim->uniform->dye_texel_size[0] =
			1.0F / (float)OWL_DYE_RESOLUTION;
		sim->uniform->dye_texel_size[1] =
			1.0F / (float)OWL_DYE_RESOLUTION;
		...
		vkCmdBindDescriptorSets(command_buffer,
					...
					r->fluid_simulation_pipeline_layout, 0,
					1, &descriptor_sets[0], 0, NULL);
		...
		vkCmdBindDescriptorSets(command_buffer,
					VK_PIPELINE_BIND_POINT_COMPUTE,
					r->fluid_simulation_pipeline_layout, 1,
					1, &descriptor_sets[1], 0, NULL);

		vkCmdBindDescriptorSets(command_buffer,
					VK_PIPELINE_BIND_POINT_COMPUTE,
					r->fluid_simulation_pipeline_layout, 3,
					1, &descriptor_sets[3], 0, NULL);
		...
		vkCmdDispatch(command_buffer, OWL_SIM_RESOLUTION,
			      OWL_SIM_RESOLUTION, 1);
