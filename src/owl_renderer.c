...
static int owl_renderer_validate_extensions(struct owl_renderer *r)
{
	...
	device_extension_markers =
		OWL_MALLOC(OWL_ARRAY_SIZE(device_extensions) *
	...
	for (i = 0; i < num_supported_extensions; ++i) {
		...
		for (j = 0; j < OWL_ARRAY_SIZE(device_extensions); ++j) {
			...
			if (0 == OWL_STRNCMP(supported_extension,
					     required_extension,
...
static int owl_renderer_find_depth_format(struct owl_renderer *r)
{
	...
	vkGetPhysicalDeviceFormatProperties(
		device, VK_FORMAT_D32_SFLOAT_S8_UINT, &format_properties);
	if (VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT &
	    format_properties.optimalTilingFeatures) {
...
static int owl_renderer_select_physical_device(struct owl_renderer *r)
{
	...
	vk_result =
		vkEnumeratePhysicalDevices(r->instance, &num_devices, devices);
	if (vk_result)
		goto cleanup;
...
static int owl_renderer_init_attachments(struct owl_renderer *r)
{
...

	{
		...
		vkGetImageMemoryRequirements(device, r->color_image,
					     &requirements);
		...
		vk_result =
			vkAllocateMemory(device, &info, NULL, &r->color_memory);
		if (vk_result)
			goto error_destroy_color_image;

		vk_result = vkBindImageMemory(device, r->color_image,
					      r->color_memory, 0);
		if (vk_result)
...

	{
		...
		vk_result =
			vkCreateImage(r->device, &info, NULL, &r->depth_image);
		if (vk_result)
...

	{
		...
		vkGetImageMemoryRequirements(device, r->depth_image,
					     &requirements);
		...
		vk_result =
			vkAllocateMemory(device, &info, NULL, &r->depth_memory);
		if (vk_result)
			goto error_destroy_depth_image;

		vk_result = vkBindImageMemory(device, r->depth_image,
					      r->depth_memory, 0);
		if (vk_result)
...
static int owl_renderer_init_render_passes(struct owl_renderer *r)
{
	...
	vk_result =
		vkCreateRenderPass(device, &info, NULL, &r->main_render_pass);
	if (vk_result)
		goto error;
...
static int owl_renderer_init_swapchain(struct owl_renderer *r)
{
...

	{
		...
		vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
			physical_device, r->surface, &capabilities);
		...
		vk_result = vkGetSwapchainImagesKHR(
			device, r->swapchain, &r->num_swapchain_images, NULL);
		if (vk_result ||
		    OWL_MAX_SWAPCHAIN_IMAGES <= r->num_swapchain_images)
...
error_destroy_framebuffers:
	for (i = i - 1; i >= 0; --i)
		vkDestroyFramebuffer(device, r->swapchain_framebuffers[i],
				     NULL);
...
static int owl_renderer_init_layouts(struct owl_renderer *r)
{
...

	{
		...
		vk_result = vkCreateDescriptorSetLayout(
			device, &info, NULL,
			&r->common_uniform_descriptor_set_layout);
		if (vk_result)
			goto error;
	}

	{
		...
		vk_result = vkCreateDescriptorSetLayout(
			device, &info, NULL,
			&r->common_texture_descriptor_set_layout);
		if (vk_result)
...

	{
		...
		vk_result = vkCreateDescriptorSetLayout(
			device, &info, NULL,
			&r->model_uniform_descriptor_set_layout);
		if (vk_result)
...

	{
		...
		vk_result = vkCreateDescriptorSetLayout(
			device, &info, NULL,
			&r->model_maps_descriptor_set_layout);
		if (vk_result)
...
error_destroy_model_maps_descriptor_set_layout:
	vkDestroyDescriptorSetLayout(device,
				     r->model_maps_descriptor_set_layout, NULL);

error_destroy_model_storage_descriptor_set_layout:
	vkDestroyDescriptorSetLayout(
		device, r->model_storage_descriptor_set_layout, NULL);

error_destroy_model_uniform_descriptor_set_layout:
	vkDestroyDescriptorSetLayout(
		device, r->model_uniform_descriptor_set_layout, NULL);
...
static int owl_renderer_init_graphics_pipelines(struct owl_renderer *r)
{
	...
	vertex_attributes[0].offset =
		offsetof(struct owl_common_vertex, position);
	...
	color_attachment.dstAlphaBlendFactor =
		VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
					   ...
					   uint64_t size)
{
...

	{
		...
		vk_result = vkMapMemory(device, r->vertex_buffer_memory, 0,
					size, 0, &r->vertex_buffer_data);
		if (vk_result)
			goto error_free_memory;
	}
...
static int owl_renderer_init_index_buffer(struct owl_renderer *r, uint64_t size)
{
	int32_t i;
	VkDevice const device = r->device;
...

	{
		...
		vk_result = vkMapMemory(device, r->index_buffer_memory, 0, size,
					0, &r->index_buffer_data);
		if (vk_result)
			goto error_free_memory;
	}
					    ...
					    uint64_t size)
{
...

	{
		...
		vk_result = vkMapMemory(device, r->uniform_buffer_memory, 0,
					size, 0, &r->uniform_buffer_data);
		if (vk_result)
			goto error_free_memory;
	}
	...
	for (i = 0; i < (int32_t)r->num_frames; ++i) {
		{
			...
			r->uniform_model_descriptor_sets[i] =
				descriptor_sets[1];
		}

		{
			...
			descriptors[0].range =
				sizeof(struct owl_common_uniform);
			...
			vkUpdateDescriptorSets(device, OWL_ARRAY_SIZE(writes),
					       writes, 0, NULL);
		}
	}
...
static int owl_renderer_init_frames(struct owl_renderer *r)
{
	...
	for (i = 0; i < (int32_t)r->num_frames; ++i) {
		...
		vk_result = vkAllocateCommandBuffers(
			device, &info, &r->submit_command_buffers[i]);
		if (vk_result)
...
#include "owl_prefilter.frag.spv.u32"
	...
	for (i = 0; i < OWL_NUM_ENVIRONMENT_MAPS; ++i) {
...

		{
			...
			vk_result =
				vkCreateImage(device, &info, NULL, images[i]);
			if (vk_result)
				goto error;
		}

		{
			...
			vkGetImageMemoryRequirements(device, *images[i],
						     &requirements);
			...
			vk_result = vkBindImageMemory(device, *images[i],
						      *memories[i], 0);
			if (vk_result)
				goto error;
		}
...

		{
			...
			vk_result = vkCreateImage(device, &info, NULL,
						  &offscreen_image);
			if (vk_result)
				goto error;
		}
      ...
      goto error;
...

		{
			...
			vk_result = vkCreateShaderModule(
				device, &info, NULL, &offscreen_vertex_shader);
			if (vk_result)
				goto error;
		}
...

		{
			...
			vertex_binding.stride =
				sizeof(struct owl_skybox_vertex);
			...
			color_attachment.blendEnable = VK_FALSE;
			color_attachment.srcColorBlendFactor =
				VK_BLEND_FACTOR_ZERO;
			color_attachment.dstColorBlendFactor =
				VK_BLEND_FACTOR_ZERO;
			color_attachment.colorBlendOp = VK_BLEND_OP_ADD;
			color_attachment.srcAlphaBlendFactor =
				VK_BLEND_FACTOR_ZERO;
			color_attachment.dstAlphaBlendFactor =
				VK_BLEND_FACTOR_ZERO;
			color_attachment.alphaBlendOp = VK_BLEND_OP_ADD;
		...
		for (j = 0; j < mips; ++j) {
			...
			for (k = 0; k < 6; ++k) {
				...
				if (OWL_IRRADIANCE_MAP == i) {
					...
					near = 0.1F;
					far = 512.0F;
					owl_m4_perspective(fov, ratio, near,
							   far, perspective);
					owl_m4_multiply(perspective,
							matrices[k],
							push_constant.mvp);

					push_constant.delta_phi =
						2.0F * OWL_PI / 180.0F;
					push_constant.delta_theta =
						0.5F * OWL_PI / 64.0F;
...

				{
                     ...
                     * 2----3
                     */
					static struct owl_skybox_vertex const
						vertices[] = {
							{ -1.0F, -1.0F,
							  -1.0F }, /* 0 */
							{ 1.0F, -1.0F,
					...
					mapped_indices =
						owl_renderer_index_allocate(
							r, sizeof(indices),
							&index_allocation);
					if (!mapped_indices)
						goto error;

					OWL_MEMCPY(mapped_vertices, vertices,
						   sizeof(vertices));
					OWL_MEMCPY(mapped_indices, indices,
						   sizeof(indices));
...

				{
					...
					vkCmdCopyImage(
						command_buffer, offscreen_image,
						VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
						*images[i],
						VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
						1, &copy);
...
*/
		...
		vkDestroyPipelineLayout(device, offscreen_pipeline_layout,
					NULL);
...
OWLAPI int owl_renderer_begin_frame(struct owl_renderer *r)
{
...

	{
		...
		vk_result = vkWaitForFences(device, 1, &in_flight_fence,
					    VK_TRUE, timeout);
		if (vk_result)
			return OWL_ERROR_FATAL;
...
OWLAPI int owl_renderer_end_frame(struct owl_renderer *r)
{
...

	{
		...
		vk_result = vkQueueSubmit(r->graphics_queue, 1, &info,
					  in_flight_fence);
		if (vk_result)
			return OWL_ERROR_FATAL;
	}
			     ...
			     struct owl_renderer_upload_allocation *alloc)
{
...

	{
		...
		vk_result =
			vkCreateBuffer(device, &info, NULL, &r->upload_buffer);
		if (vk_result)
			goto error;
	}

	{
		...
		vkGetBufferMemoryRequirements(device, r->upload_buffer,
					      &requirements);
...
OWLAPI int owl_renderer_load_skybox(struct owl_renderer *r, char const *path)
{
...

	{
		...
		vk_result = vkAllocateDescriptorSets(
			device, &info, &r->environment_descriptor_set);
		if (vk_result) {
			ret = OWL_ERROR_FATAL;
...

	{
		...
		vkUpdateDescriptorSets(device, OWL_ARRAY_SIZE(writes), writes,
				       0, NULL);
	}
