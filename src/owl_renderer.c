...
static int owl_renderer_init_instance(struct owl_renderer *r) {
...
#if defined(OWL_ENABLE_VALIDATION)
  r->vk_create_debug_utils_messenger_ext = (PFN_vkCreateDebugUtilsMessengerEXT)
  ...
  r->vk_debug_marker_set_object_name_ext = (PFN_vkDebugMarkerSetObjectNameEXT)
      vkGetInstanceProcAddr(r->instance, "vkDebugMarkerSetObjectNameEXT");
  if (!r->vk_destroy_debug_utils_messenger_ext)
    goto error_destroy_instance;
...
static int owl_renderer_select_physical_device(struct owl_renderer *r) {
  ...
  for (i = 0; i < num_devices && !device_found; ++i) {
    ...
    for (current_family = 0;
      ...
      vkGetPhysicalDeviceSurfaceSupportKHR(device, current_family, r->surface,
                                           &has_surface);
