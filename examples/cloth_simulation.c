...
static struct owl_cloth_simulation *simulation;
...
main(void) {
  ...
  CHECK(owl_cloth_simulation_init(simulation, renderer, 20, 20,
                                  "../../assets/cloth.jpeg"));
  ...
  while (!owl_plataform_should_close(window)) {
    ...
    CHECK(owl_renderer_begin_frame(renderer));
    {
      ...
      owl_cloth_simulation_update(simulation, time_stamp - prev_time_stamp);

      owl_draw_cloth_simulation(renderer, simulation);
  ...
  owl_cloth_simulation_deinit(simulation, renderer);
  free(simulation);
