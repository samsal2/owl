...
#define OWL_FIRST_CHAR ((owl_i32)(' '))
#define OWL_NUM_CHARS ((owl_i32)('~' - ' '))
...
struct owl_vk_renderer {
  ...
  owl_u32 swapchain_width;
  owl_u32 swapchain_height;
  owl_u32 swapchain_image;
  owl_u32 num_swapchain_images;
  ...
  owl_u32 upload_heap_in_use;

  VkSampler linear_sampler;

  owl_b32 skybox_loaded;
  ...
  owl_u32 num_frame_garbage_buffers[OWL_MAX_SWAPCHAIN_IMAGES];
  OWL_DEF_FRAME_GARBAGE_ARRAY(VkBuffer, frame_garbage_buffers);

  owl_u32 num_frame_garbage_memories[OWL_MAX_SWAPCHAIN_IMAGES];
  OWL_DEF_FRAME_GARBAGE_ARRAY(VkDeviceMemory, frame_garbage_memories);

  owl_u32 num_frame_garbage_descriptor_sets[OWL_MAX_SWAPCHAIN_IMAGES];
