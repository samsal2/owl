...
void owl_ui_renderer_stats_draw(struct owl_ui_renderer_state *rs,
  ...
  snprintf(buffer, sizeof(buffer),"framerate: %.2f fps", 1.0 / r->time_stamp_delta);
  ...
  for (i = 0; i < OWL_RENDERER_IN_FLIGHT_FRAMES_COUNT; ++i) {
    snprintf(buffer, sizeof(buffer), "frame_heap_offset[%i]: %llu bytes", i, r->frame_heap_offsets[i]);

    cmd.position[0] = -1.0F;
    cmd.position[1] = -0.98F * (1 - (0.08 * (i + 1)));
    cmd.position[2] = 0.0F;
    cmd.text = buffer;
  ...
  snprintf(buffer, sizeof(buffer), "frame_heap_buffer_size: %llu bytes", r->frame_heap_buffer_size);
