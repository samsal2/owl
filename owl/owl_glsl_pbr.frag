...
microfacetDistribution (PBRInfo pbrInputs)
{
  ...
  float f = (pbrInputs.NdotH * roughnessSq - pbrInputs.NdotH) * pbrInputs.NdotH
            + 1.0;
...
main ()
{
    ...
    if (material.baseColorTextureSet > -1)
    {
      baseColor
          = SRGBtoLINEAR (texture (
                colorMap, material.baseColorTextureSet == 0 ? inUV0 : inUV1))
      ...
      perceptualRoughness
          = 1.0
            - texture (physicalDescriptorMap,
                       material.physicalDescriptorTextureSet == 0 ? inUV0
                                                                  : inUV1)
                  .a;
    }
    else
  ...
  vec3 l = normalize (
      uboParams.lightDir.xyz);         // Vector from surface point to light
  ...
  {
    float ao
        = texture (aoMap, (material.occlusionTextureSet == 0 ? inUV0 : inUV1))
              .r;
  ...
  {
    vec3 emissive
        = SRGBtoLINEAR (texture (emissiveMap, material.emissiveTextureSet == 0
                                                  ? inUV0
                                                  : inUV1))
              .rgb
          * u_EmissiveFactor;
  ...
  if (uboParams.debugViewInputs > 0.0)
  {
    ...
    case 1:
      outColor.rgba
          = material.baseColorTextureSet > -1 ? texture (
                colorMap, material.baseColorTextureSet == 0 ? inUV0 : inUV1)
                                              : vec4 (1.0f);
      break;
    case 2:
