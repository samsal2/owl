...
OWL_GLOBAL char const *const g_required_device_extensions[] = {VK_KHR_SWAPCHAIN_EXTENSION_NAME};

#ifdef OWL_ENABLE_VALIDATION

OWL_GLOBAL char const *const g_debug_validation_layers[] = {"VK_LAYER_KHRONOS_validation"};

#endif /* OWL_ENABLE_VALIDATION */

OWL_INTERNAL enum owl_code owl_renderer_init_instance_(struct owl_renderer_init_info const *info,
...
#define OWL_GET_INSTANCE_PROC_ADDR(i, fn) ((PFN_##fn)vkGetInstanceProcAddr((i), #fn))

#include <stdio.h>
static VKAPI_ATTR VKAPI_CALL VkBool32 owl_vk_debug_callback_(VkDebugUtilsMessageSeverityFlagBitsEXT severity,
  ...
  if (severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT)
    fprintf(stderr, "\033[1;32m[VALIDATION_LAYER]\033[0m \033[1;36m(VERBOSE)\033[0m %s\n", data->pMessage);
  else if (severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT)
    fprintf(stderr, "\033[1;32m[VALIDATION_LAYER]\033[0m \033[1;34m(INFO)\033[0m %s\n", data->pMessage);
  else if (severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT)
    fprintf(stderr, "\033[1;32m[VALIDATION_LAYER]\033[0m \033[1;33m(WARNING)\033[0m %s\n", data->pMessage);
  else if (severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)
    fprintf(stderr, "\033[1;32m[VALIDATION_LAYER]\033[0m \033[1;31m(ERROR)\033[0m %s\n", data->pMessage);
...
OWL_INTERNAL enum owl_code owl_renderer_init_debug_(struct owl_renderer *r) {
  ...
  vkCreateDebugUtilsMessengerEXT = OWL_GET_INSTANCE_PROC_ADDR(r->instance, vkCreateDebugUtilsMessengerEXT);
...
#undef OWL_DEBUG_TYPE_FLAGS
  ...
  OWL_VK_CHECK(vkCreateDebugUtilsMessengerEXT(r->instance, &debug_messenger, NULL, &r->debug_messenger));
...
OWL_INTERNAL void owl_renderer_deinit_debug_(struct owl_renderer *r) {
  ...
  vkDestroyDebugUtilsMessengerEXT = OWL_GET_INSTANCE_PROC_ADDR(r->instance, vkDestroyDebugUtilsMessengerEXT);
...
OWL_INTERNAL enum owl_code owl_renderer_init_surface_(struct owl_renderer_init_info const *info,
                                                      struct owl_renderer *r) {
...
OWL_INTERNAL enum owl_code owl_renderer_fill_device_options_(struct owl_renderer *r) {
  enum owl_code code = OWL_SUCCESS;

  OWL_VK_CHECK(vkEnumeratePhysicalDevices(r->instance, &r->device_options_count, NULL));
  ...
  OWL_VK_CHECK(vkEnumeratePhysicalDevices(r->instance, &r->device_options_count, r->device_options));

end:
  return code;
}

OWL_INTERNAL int owl_renderer_query_families_(struct owl_renderer const *r, owl_u32 *graphics_family_index,
...
#define OWL_QUEUE_UNSELECTED (owl_u32) - 1
  ...
  vkGetPhysicalDeviceQueueFamilyProperties(r->physical_device, &count, properties);
  ...
  *present_family_index = OWL_QUEUE_UNSELECTED;
  ...
  for (i = 0; i < count; ++i) {
    ...
    OWL_VK_CHECK(vkGetPhysicalDeviceSurfaceSupportKHR(r->physical_device, i, r->surface, &has_surface));
...
OWL_INTERNAL int owl_validate_device_extensions_(owl_u32 count, VkExtensionProperties const *extensions) {
  owl_u32 i;
  int found = 1;
...
OWL_INTERNAL enum owl_code owl_renderer_select_physical_device_(struct owl_renderer *r) {
  owl_u32 i;
  enum owl_code code = OWL_SUCCESS;
  ...
  for (i = 0; i < r->device_options_count; ++i) {
    ...
    OWL_VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(r->physical_device, r->surface, &has_formats, NULL));

    if (!has_formats)
      continue;

    OWL_VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(r->physical_device, r->surface, &has_modes, NULL));

    if (!has_modes)
      continue;

    if (!owl_renderer_query_families_(r, &r->graphics_family_index, &r->present_family_index))
      continue;
    ...
    OWL_VK_CHECK(vkEnumerateDeviceExtensionProperties(r->physical_device, NULL, &extension_count, NULL));
    ...
    OWL_VK_CHECK(vkEnumerateDeviceExtensionProperties(r->physical_device, NULL, &extension_count, extensions));
    ...
    vkGetPhysicalDeviceMemoryProperties(r->physical_device, &r->device_memory_properties);
    code = OWL_SUCCESS;
    goto end;
  }
...
OWL_INTERNAL enum owl_code owl_renderer_select_surface_format_(struct owl_renderer *r,
  ...
  OWL_VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(r->physical_device, r->surface, &formats_count, NULL));
  ...
  OWL_VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(r->physical_device, r->surface, &formats_count, formats));
...
OWL_INTERNAL enum owl_code owl_renderer_select_sample_count_(struct owl_renderer *r) {
  enum owl_code code = OWL_SUCCESS;
  VkSampleCountFlags const samples = r->device_properties.limits.framebufferColorSampleCounts &
...
OWL_INTERNAL void owl_renderer_clamp_swapchain_extent_(owl_u32 width, owl_u32 height, struct owl_renderer *r) {
#define OWL_NO_RESTRICTIONS (owl_u32) - 1

  VkSurfaceCapabilitiesKHR capabilities;

  OWL_VK_CHECK(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(r->physical_device, r->surface, &capabilities));
...
OWL_INTERNAL enum owl_code owl_renderer_select_present_mode_(struct owl_renderer *r, VkPresentModeKHR mode) {
#define OWL_MAX_PRESENT_MODES 16
  ...
  OWL_VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(r->physical_device, r->surface, &count, NULL));
  ...
  OWL_VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(r->physical_device, r->surface, &count, modes));
...
OWL_INTERNAL enum owl_code owl_renderer_init_swapchain_(struct owl_renderer_init_info const *info,
                                                        struct owl_renderer *r) {
  owl_u32 families[2];
  VkSurfaceCapabilitiesKHR capabilities;
  ...
  owl_renderer_clamp_swapchain_extent_((owl_u32)info->framebuffer_width, (owl_u32)info->framebuffer_height, r);
  ...
  OWL_VK_CHECK(vkCreateSwapchainKHR(r->device, &swapchain, NULL, &r->swapchain));

  OWL_VK_CHECK(vkGetSwapchainImagesKHR(r->device, r->swapchain, &r->swapchain_images_count, NULL));
  ...
  OWL_VK_CHECK(vkGetSwapchainImagesKHR(r->device, r->swapchain, &r->swapchain_images_count, r->swapchain_images));
...
OWL_INTERNAL enum owl_code owl_renderer_init_swapchain_views_(struct owl_renderer *r) {
  owl_u32 i;
  enum owl_code code = OWL_SUCCESS;
  ...
  for (i = 0; i < r->swapchain_images_count; ++i) {
    ...
    OWL_VK_CHECK(vkCreateImageView(r->device, &view, NULL, &r->swapchain_views[i]));
  }
...
OWL_INTERNAL enum owl_code owl_renderer_init_common_pools_(struct owl_renderer *r) {
  enum owl_code code = OWL_SUCCESS;

  {
    ...
    OWL_VK_CHECK(vkCreateCommandPool(r->device, &pool, NULL, &r->transient_command_pool));
  }
...
#define OWL_MAX_SETS 80
    ...
    OWL_VK_CHECK(vkCreateDescriptorPool(r->device, &pool, NULL, &r->common_set_pool));
#undef OWL_MAX_SETS
...
OWL_INTERNAL enum owl_code owl_renderer_init_main_render_pass_(struct owl_renderer *r) {
...
#define OWL_SRC_STAGE (VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT)

#define OWL_DST_STAGE (VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT)

#define OWL_DST_ACCESS (VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT)
...
#undef OWL_SRC_STAGE
  ...
  OWL_VK_CHECK(vkCreateRenderPass(r->device, &render_pass, NULL, &r->main_render_pass));

  return code;
}

OWL_INTERNAL void owl_renderer_deinit_main_render_pass_(struct owl_renderer *r) {
  vkDestroyRenderPass(r->device, r->main_render_pass, NULL);
}

OWL_INTERNAL enum owl_code owl_renderer_init_attachments_(struct owl_renderer *r) {
  enum owl_code code = OWL_SUCCESS;

  {
#define OWL_IMAGE_USAGE VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT
...
#undef OWL_IMAGE_USAGE
...

  {
    ...
    memory.memoryTypeIndex =
        owl_renderer_find_memory_type(r, requirements.memoryTypeBits, OWL_MEMORY_VISIBILITY_GPU_ONLY);

    OWL_VK_CHECK(vkAllocateMemory(r->device, &memory, NULL, &r->color_memory));

    OWL_VK_CHECK(vkBindImageMemory(r->device, r->color_image, r->color_memory, 0));
  }
...

  {
    ...
    OWL_VK_CHECK(vkCreateImage(r->device, &image, NULL, &r->depth_stencil_image));
  }

  {
    VkMemoryRequirements requirements;
    VkMemoryAllocateInfo memory;
    vkGetImageMemoryRequirements(r->device, r->depth_stencil_image, &requirements);
    ...
    OWL_VK_CHECK(vkAllocateMemory(r->device, &memory, NULL, &r->depth_stencil_memory));

    OWL_VK_CHECK(vkBindImageMemory(r->device, r->depth_stencil_image, r->depth_stencil_memory, 0));
  }

  {
    ...
    OWL_VK_CHECK(vkCreateImageView(r->device, &view, NULL, &r->depth_stencil_view));
  }
...
OWL_INTERNAL enum owl_code owl_renderer_init_swapchain_framebuffers_(struct owl_renderer *r) {
  owl_u32 i;
  enum owl_code code = OWL_SUCCESS;
  ...
  for (i = 0; i < r->swapchain_images_count; ++i) {
    ...
    OWL_VK_CHECK(vkCreateFramebuffer(r->device, &framebuffer, NULL, &r->swapchain_framebuffers[i]));
  }
...
OWL_INTERNAL enum owl_code owl_renderer_init_set_layouts_(struct owl_renderer *r) {
  enum owl_code code = OWL_SUCCESS;

  {
    ...
    OWL_VK_CHECK(vkCreateDescriptorSetLayout(r->device, &layout, NULL, &r->pvm_set_layout));
  }

  {
    ...
    OWL_VK_CHECK(vkCreateDescriptorSetLayout(r->device, &layout, NULL, &r->texture_set_layout));
  }

  {
    ...
    OWL_VK_CHECK(vkCreateDescriptorSetLayout(r->device, &layout, NULL, &r->scene_set_layout));
  }

  {
    ...
    OWL_VK_CHECK(vkCreateDescriptorSetLayout(r->device, &layout, NULL, &r->material_set_layout));
  }

  {
    ...
    OWL_VK_CHECK(vkCreateDescriptorSetLayout(r->device, &layout, NULL, &r->node_set_layout));
  }
...
OWL_INTERNAL enum owl_code owl_renderer_init_pipelines_layouts_(struct owl_renderer *r) {
  enum owl_code code = OWL_SUCCESS;
...

  {
    ...
    OWL_VK_CHECK(vkCreatePipelineLayout(r->device, &layout, NULL, &r->common_pipeline_layout));
  }

  {
    ...
    OWL_VK_CHECK(vkCreatePipelineLayout(r->device, &layout, NULL, &r->pbr_pipeline_layout));
  }

  return code;
}

OWL_INTERNAL void owl_renderer_deinit_pipeline_layouts_(struct owl_renderer *r) {
...
#include "../shaders/basic_vertex.spv.u32"
    ...
    OWL_VK_CHECK(vkCreateShaderModule(r->device, &shader, NULL, &r->basic_vertex_shader));
  }
...
#include "../shaders/basic_fragment.spv.u32"
    ...
    OWL_VK_CHECK(vkCreateShaderModule(r->device, &shader, NULL, &r->basic_fragment_shader));
  }
...
#include "../shaders/font_fragment.spv.u32"
    ...
    OWL_VK_CHECK(vkCreateShaderModule(r->device, &shader, NULL, &r->font_fragment_shader));
  }
...
#include "../shaders/skybox_vertex.spv.u32"
    ...
    OWL_VK_CHECK(vkCreateShaderModule(r->device, &shader, NULL, &r->skybox_vertex_shader));
  }
...
#include "../shaders/skybox_fragment.spv.u32"
    ...
    OWL_VK_CHECK(vkCreateShaderModule(r->device, &shader, NULL, &r->skybox_fragment_shader));
  }
...
#include "../shaders/pbr_vertex.spv.u32"
    ...
    OWL_VK_CHECK(vkCreateShaderModule(r->device, &shader, NULL, &r->pbr_vertex_shader));
  }
...
#include "../shaders/pbr_fragment.spv.u32"
    ...
    OWL_VK_CHECK(vkCreateShaderModule(r->device, &shader, NULL, &r->pbr_fragment_shader));
  }
...
OWL_INTERNAL enum owl_code owl_renderer_init_pipelines_(struct owl_renderer *r) {
...
#define OWL_MAX_COLOR_ATTACHMENTS_COUNT 8
  ...
  VkVertexInputBindingDescription vertex_bindings[OWL_MAX_VERTEX_INPUT_BINDINGS_COUNT];
  VkVertexInputAttributeDescription vertex_attributes[OWL_MAX_VERTEX_INPUT_ATTRIBUTES_COUNT];
  ...
  VkPipelineColorBlendAttachmentState color_blend_attachments[OWL_MAX_COLOR_ATTACHMENTS_COUNT];
...
#undef OWL_COLOR_WRITE_MASK
    ...
    case OWL_PIPELINE_TYPE_PBR:
      ...
      OWL_MEMSET(&depth_stencil_state.front, 0, sizeof(depth_stencil_state.front));
    ...
    OWL_VK_CHECK(vkCreateGraphicsPipelines(r->device, VK_NULL_HANDLE, 1, &pipeline, NULL, &r->pipelines[i]));
  }
...
OWL_INTERNAL enum owl_code owl_renderer_init_frame_commands_(struct owl_renderer *r) {
  owl_u32 i;
  enum owl_code code = OWL_SUCCESS;
  ...
  for (i = 0; i < OWL_RENDERER_DYNAMIC_BUFFER_COUNT; ++i) {
    ...
    OWL_VK_CHECK(vkCreateCommandPool(r->device, &command_pool, NULL, &r->frame_command_pools[i]));
  }
  ...
  for (i = 0; i < OWL_RENDERER_DYNAMIC_BUFFER_COUNT; ++i) {
    ...
    OWL_VK_CHECK(vkAllocateCommandBuffers(r->device, &command, &r->frame_command_buffers[i]));
  }
...
OWL_INTERNAL void owl_renderer_deinit_frame_commands_(struct owl_renderer *r) {
  ...
  for (i = 0; i < OWL_RENDERER_DYNAMIC_BUFFER_COUNT; ++i)
    vkFreeCommandBuffers(r->device, r->frame_command_pools[i], 1, &r->frame_command_buffers[i]);

  for (i = 0; i < OWL_RENDERER_DYNAMIC_BUFFER_COUNT; ++i)
    vkDestroyCommandPool(r->device, r->frame_command_pools[i], NULL);
}

OWL_INTERNAL enum owl_code owl_renderer_init_frame_sync_(struct owl_renderer *r) {
  owl_u32 i;
  enum owl_code code = OWL_SUCCESS;
  ...
  for (i = 0; i < OWL_RENDERER_DYNAMIC_BUFFER_COUNT; ++i) {
    ...
    OWL_VK_CHECK(vkCreateFence(r->device, &fence, NULL, &r->in_flight_fences[i]));
  }
  ...
  for (i = 0; i < OWL_RENDERER_DYNAMIC_BUFFER_COUNT; ++i) {
    ...
    semaphore.flags = 0;
    OWL_VK_CHECK(vkCreateSemaphore(r->device, &semaphore, NULL, &r->render_done_semaphores[i]));
  }
  ...
  for (i = 0; i < OWL_RENDERER_DYNAMIC_BUFFER_COUNT; ++i) {
    ...
    semaphore.pNext = NULL;
    semaphore.flags = 0;
    OWL_VK_CHECK(vkCreateSemaphore(r->device, &semaphore, NULL, &r->image_available_semaphores[i]));
  }
...
OWL_INTERNAL void owl_renderer_deinit_garbage_(struct owl_renderer *r) {
  ...
  for (i = 0; i < r->garbage_pvm_sets_count; ++i)
    vkFreeDescriptorSets(r->device, r->common_set_pool, 1, &r->garbage_pvm_sets[i]);
...
OWL_INTERNAL enum owl_code owl_renderer_init_dynamic_buffer_(struct owl_renderer *r, VkDeviceSize size) {
  owl_u32 i;
  enum owl_code code = OWL_SUCCESS;
...
#define OWL_DOUBLE_BUFFER_USAGE                                                                                        \
  (VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT |           \
   VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT)
    ...
    for (i = 0; i < OWL_RENDERER_DYNAMIC_BUFFER_COUNT; ++i)
      OWL_VK_CHECK(vkCreateBuffer(r->device, &buffer, NULL, &r->dynamic_buffers[i]));
...
#undef OWL_DOUBLE_BUFFER_USAGE
  ...
  /* init memory */
  {
    ...
    vkGetBufferMemoryRequirements(r->device, r->dynamic_buffers[0], &requirements);
    ...
    memory.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    memory.pNext = NULL;
    memory.allocationSize = r->dynamic_buffer_aligned_size * OWL_RENDERER_DYNAMIC_BUFFER_COUNT;
    memory.memoryTypeIndex =
        owl_renderer_find_memory_type(r, requirements.memoryTypeBits, OWL_MEMORY_VISIBILITY_CPU_ONLY);

    OWL_VK_CHECK(vkAllocateMemory(r->device, &memory, NULL, &r->dynamic_memory));
  }
  ...
  /* bind buffers to memory */
  {
    for (i = 0; i < OWL_RENDERER_DYNAMIC_BUFFER_COUNT; ++i)
      OWL_VK_CHECK(
          vkBindBufferMemory(r->device, r->dynamic_buffers[i], r->dynamic_memory, i * r->dynamic_buffer_aligned_size));
  }

  /* map memory */
  {
    void *data;
    OWL_VK_CHECK(vkMapMemory(r->device, r->dynamic_memory, 0, VK_WHOLE_SIZE, 0, &data));

    for (i = 0; i < OWL_RENDERER_DYNAMIC_BUFFER_COUNT; ++i)
      r->dynamic_data[i] = (owl_byte *)data + i * r->dynamic_buffer_aligned_size;
  }
  ...
  /* init pvm descriptor sets */
  {
    ...
    OWL_VK_CHECK(vkAllocateDescriptorSets(r->device, &sets, r->dynamic_pvm_sets));
  }
...
OWL_INTERNAL void owl_renderer_deinit_dynamic_buffer_(struct owl_renderer *r) {
  ...
  vkFreeDescriptorSets(r->device, r->common_set_pool, OWL_ARRAY_SIZE(r->dynamic_pvm_sets), r->dynamic_pvm_sets);
...
#if 0
OWL_INTERNAL enum owl_code owl_renderer_init_lutbrdf_resources_(struct owl_renderer *r) {
...
#define OWL_BRDFLUT_USAGE VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT
  {
    VkImageCreateInfo image;
...

  {
    ...
    OWL_VK_CHECK(vkAllocateMemory(r->device, &memory, NULL, &r->lutbrdf_memory));
    OWL_VK_CHECK(vkBindImageMemory(r->device, r->lutbrdf_image, r->lutbrdf_memory, 0));
  }
...

  {
    ...
    OWL_VK_CHECK(vkCreateSampler(r->device, &sampler, NULL, &r->lutbrdf_sampler));
  }

  {
    ...
    OWL_VK_CHECK(vkCreateRenderPass(r->device, &render_pass, NULL, &lutbrdf_render_pass));
  }

  {
    ...
    OWL_VK_CHECK(vkCreateFramebuffer(r->device, &framebuffer, NULL, &lutbrdf_framebuffer));
  }

  {
    ...
    OWL_VK_CHECK(vkCreateDescriptorSetLayout(r->device, &layout, NULL, &lutbrdf_set_layout));
  }

  {
    ...
    OWL_VK_CHECK(vkCreatePipelineLayout(r->device, &layout, NULL, &lutbrdf_pipeline_layout));
  }
...
#include "../shaders/brdflut_vertex.spv.u32"
    ...
    OWL_VK_CHECK(vkCreateShaderModule(r->device, &shader, NULL, &lutbrdf_vertex_shader));
  }
...
#include "../shaders/brdflut_fragment.spv.u32"
    ...
    OWL_VK_CHECK(vkCreateShaderModule(r->device, &shader, NULL, &lutbrdf_fragment_shader));
  }
...
#undef OWL_COLOR_WRITE_MASK
    ...
    OWL_VK_CHECK(vkCreateGraphicsPipelines(r->device, VK_NULL_HANDLE, 1, &pipeline, NULL, &lutbrdf_pipeline));
  }

  {
    ...
    {
      vkCmdBindPipeline(command, VK_PIPELINE_BIND_POINT_GRAPHICS, lutbrdf_pipeline);
...
OWL_INTERNAL void owl_renderer_deinit_lutbrdf_resources_(struct owl_renderer *r) {
...
enum owl_cubemap_target { OWL_CUBEMAP_TARGET_IRRADIANCE 0 OWL_CUBEMAP_TARGET_PREFILTERED 1 };

#define OWL_CUBEMAP_TARGET_COUNT 2

OWL_INTERNAL enum owl_code owl_renderer_init_irradiance_and_prefiltered_cubemaps_(struct owl_renderer *r) {
  int i;
  ...
  for (i = 0; i < OWL_CUBEMAP_TARGET_COUNT; ++i) {
    ...
    {
#define OWL_IMAGE_USAGE VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
      VkImageCreateInfo image;
      ...
      image.samples = VK_SAMPLE_COUNT_1_BIT;
      image.tiling = VK_IMAGE_TILING_OPTIMAL;
      image.usage = OWL_IMAGE_USAGE : image.sharingMode = VK_SHARING_MODEL_EXCLUSIVE;
...
#undef OWL_IMAGE_USAGE
...

    {
      ...
      memory.allocationSize = requirements.size;
      memory.memoryTypeIndex =
          owl_renderer_find_memory_type(r->device, requirements.memoryTypeBits, OWL_MEMORY_VISIBILITY_GPU_ONLY);
...

    {
      ...
      OWL_VK_CHECK(vkCreateSampler(r->device, &sampler, NULL, &r->current_sampler));
    }

    {
      ...
      OWL_VK_CHECK(vkCreateRenderPass(r->device, &render_pass, NULL, &current_render_pass));
    }

    {
#define OWL_IMAGE_USAGE VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
      VkImageCreateInfo image;
...
#undef OWL_IMAGE_USAGE
...

    {
      ...
      OWL_VK_CHECK(vkCreateFramebuffer(r->device, &framebuffer, NULL < &offscreen_framebuffer));
    }

    {
      ...
      vkCmdPipelineBarrier(command, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0, NULL,
                           0, NULL, 1, &barrier);
...

    {
      ...
      OWL_VK_CHECK(vkCreateDescriptorSetLayout(r->device, &layout, NULL, &current_set_layout));
    }

    {
      ...
      OWL_VK_CHECK(vkCreatePipelineLayout(r->device, &layout, NULL, &current_pipeline_layout));
    }
...
OWL_INTERNAL enum owl_code owl_renderer_move_dynamic_to_garbage_(struct owl_renderer *r) {
  int i;
  enum owl_code code = OWL_SUCCESS;
...
OWL_INTERNAL enum owl_code owl_renderer_reserve_dynamic_memory_(struct owl_renderer *r, VkDeviceSize size) {
  enum owl_code code = OWL_SUCCESS;
...
OWL_INTERNAL enum owl_code owl_renderer_reinit_swapchain_(struct owl_renderer_init_info const *info,
                                                          struct owl_renderer *r) {
  enum owl_code code = OWL_SUCCESS;
...
void *owl_renderer_dynamic_buffer_alloc(struct owl_renderer *r, VkDeviceSize size,
  ...
  r->dynamic_offset = OWL_ALIGNU2(r->dynamic_offset + size, r->dynamic_buffer_alignment);

end:
  return data;
}

OWL_INTERNAL VkMemoryPropertyFlags owl_as_vk_memory_property_flags(enum owl_memory_visibility vis) {
  switch (vis) {
...
enum owl_code owl_renderer_bind_pipeline(struct owl_renderer *r, enum owl_pipeline_type type) {
  ...
  vkCmdBindPipeline(r->active_frame_command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, r->active_pipeline);

end:
  return code;
}

enum owl_code owl_renderer_alloc_single_use_command_buffer(struct owl_renderer const *r, VkCommandBuffer *out) {
  enum owl_code code = OWL_SUCCESS;
...
OWL_INTERNAL enum owl_code owl_renderer_acquire_next_image_(struct owl_renderer *r) {
  ...
  VkResult const result =
      vkAcquireNextImageKHR(r->device, r->swapchain, OWL_TIMEOUT, r->active_image_available_semaphore, VK_NULL_HANDLE,
  ...
  r->active_swapchain_image = r->swapchain_images[r->active_swapchain_image_index];

  r->active_swapchain_framebuffer = r->swapchain_framebuffers[r->active_swapchain_image_index];

  return code;
}

OWL_INTERNAL void owl_renderer_prepare_frame_(struct owl_renderer *r) {
  OWL_VK_CHECK(vkWaitForFences(r->device, 1, &r->active_in_flight_fence, VK_TRUE, OWL_TIMEOUT));
...
OWL_INTERNAL void owl_renderer_begin_recording_(struct owl_renderer *r) {
...

  {
    ...
    vkCmdBeginRenderPass(r->active_frame_command_buffer, &begin, VK_SUBPASS_CONTENTS_INLINE);
  }
}
...
#define OWL_WAIT_STAGE VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT
  ...
  OWL_VK_CHECK(vkQueueSubmit(r->graphics_queue, 1, &submit, r->active_in_flight_fence));
