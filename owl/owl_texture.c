...
owl_texture_init(struct owl_renderer *renderer, struct owl_texture_desc *desc,
  ...
  if (OWL_TEXTURE_SOURCE_DATA == desc->source) {
    ...
    format = owl_pixel_format_as_vk_format(desc->format);
    ...
    upload_data = owl_renderer_upload_allocate(renderer, size,
                                               &upload_allocation);
    if (!upload_data) {
    ...
    OWL_MEMCPY(upload_data, desc->pixels, size);
  } else if (OWL_TEXTURE_SOURCE_FILE == desc->source) {
    /* when loading from disk, always use r8g8b8a8_srgb */
    format = owl_pixel_format_as_vk_format(OWL_PIXEL_FORMAT_R8G8B8A8_SRGB);
    ...
    if (OWL_TEXTURE_TYPE_2D == desc->type) {
      ...
      for (i = 0; i < (int32_t)OWL_ARRAY_SIZE(names); ++i) {
        ...
        if (texture->width && texture->height) {
          ...
          upload_data = owl_renderer_upload_allocate(renderer, size * 6,
                                                     &upload_allocation);
        ...
        OWL_MEMCPY(upload_data + offset, data, size);
