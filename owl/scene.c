...
OWL_INTERNAL enum owl_code owl_scene_load_images_(struct owl_renderer *r,
  ...
  scene->images_size = (int)gltf->images_count;
...
OWL_INTERNAL enum owl_code owl_scene_load_textures_(struct owl_renderer *r,
  ...
  scene->textures_size = (int)gltf->textures_count;
...
OWL_INTERNAL enum owl_code owl_scene_load_materials_(struct owl_renderer *r,
  ...
  if (OWL_SCENE_MAX_MATERIALS_SIZE <= (int)gltf->materials_count) {
    code = OWL_ERROR_OUT_OF_BOUNDS;
    goto end;
  }

  scene->materials_size = (int)gltf->materials_count;
...
OWL_INTERNAL void
owl_scene_find_load_info_capacities_(cgltf_node const *from_node,
                                     struct owl_scene_load_info *sli) {
  int i;
  cgltf_mesh const *from_mesh = NULL;
  cgltf_attribute const *attribute = NULL;
  cgltf_accessor const *accessor = NULL;

  for (i = 0; i < (int)from_node->children_count; ++i)
    owl_scene_find_load_info_capacities_(from_node->children[i], sli);
...
owl_scene_load_node_(struct owl_renderer *r, cgltf_data const *gltf,
  ...
  node_data->mesh.slot = scene->meshes_size++;

  if (OWL_SCENE_MAX_MESHES_SIZE <= node_data->mesh.slot) {
    code = OWL_ERROR_OUT_OF_BOUNDS;
    goto end;
  }

  scene->meshes[node_data->mesh.slot].primitives_size = 0;
  ...
  if (from_node->mesh) {
    ...
    mesh_data->primitives_size = (int)from_mesh->primitives_count;
    ...
    for (i = 0; i < (int)from_mesh->primitives_count; ++i) {
      ...
      primitive->slot = scene->primitives_size++;

      if (OWL_SCENE_MAX_PRIMITIVES_SIZE <= primitive->slot) {
        code = OWL_ERROR_OUT_OF_BOUNDS;
        goto end;
      }
      ...
      if (attribute) {
        ...
        vertices_size = (int)accessor->count;
      }
      ...
      indices_size = (int)accessor->count;

      switch (accessor->component_type) {
      case cgltf_component_type_r_32u: {
        int offset = sli->indices_size;
        owl_u32 const *indices =
            (owl_u32 const *)(&b[view->offset + accessor->offset]);
        for (j = 0; j < (int)accessor->count; ++j) {
          sli->indices[offset + j] = indices[sli->vertices_size + j];
        }
      } break;
      ...
      primitive_data->first = (owl_u32)sli->indices_size;
      primitive_data->size = (owl_u32)indices_size;
      primitive_data->material.slot =
          (int)(from_primitive->material - gltf->materials);

      sli->indices_size += accessor->count;
      sli->vertices_size += vertices_size;
    }
  }
...
OWL_INTERNAL void owl_scene_load_buffers_(struct owl_renderer *r,
                                          ...
                                          struct owl_scene *scene) {
  OWL_ASSERT(sli->vertices_size == sli->vertices_capacity);
  OWL_ASSERT(sli->indices_size == sli->indices_capacity);

  {
    ...
    buffer.pNext = NULL;
    buffer.flags = 0;
    buffer.size = (owl_u64)sli->vertices_size * sizeof(struct owl_scene_vertex);
    buffer.usage =
...

  {
    ...
    buffer.pNext = NULL;
    buffer.flags = 0;
    buffer.size = (owl_u64)sli->indices_size * sizeof(owl_u32);
    buffer.usage =
...

  {
...

    {
      ...
      copy.dstOffset = 0;
      copy.size = (owl_u64)sli->vertices_size * sizeof(struct owl_scene_vertex);
...

    {
      ...
      copy.dstOffset = 0;
      copy.size = (owl_u64)sli->indices_size * sizeof(owl_u32);
...
OWL_INTERNAL enum owl_code owl_scene_load_skins_(struct owl_renderer *r,
  ...
  if (OWL_SCENE_MAX_SKINS_SIZE <= (int)gltf->skins_size) {
    code = OWL_ERROR_OUT_OF_BOUNDS;
    goto end;
  }

  scene->skins_size = (int)gltf->skins_size;

  for (i = 0; i < (int)gltf->skins_size; ++i) {
    int j;
    owl_byte const *data;
    cgltf_accessor const *accessor = NULL;
    cgltf_buffer_view const *view = NULL;
    ...
    /* verify the joint size is within capacity */
    if (OWL_SCENE_SKIN_MAX_JOINTS_SIZE <= (int)from_skin->joints_size) {
      code = OWL_ERROR_OUT_OF_BOUNDS;
      goto end;
    }

    skin_data->joints_size = (int)from_skin->joints_size;

    /* setup the joints nodes */
    for (j = 0; j < (int)from_skin->joints_size; ++i)
      skin_data->joints[i].slot = (int)(from_skin->joints[j] - gltf->nodes);

    /* set the default inverse_bind_matrices_size */
    skin_data->inverse_bind_matrices_size = 0;
    ...
    /* verify the inverse bind matrices size */
    if (OWL_SCENE_SKIN_MAX_INVERSE_BIND_MATRICES_SIZE <=
        (int)accessor->size) {
      code = OWL_ERROR_OUT_OF_BOUNDS;
      goto end;
    }

    skin_data->inverse_bind_matrices_size = (int)accessor->size;
    ...
    /* create the ssbo_buffer */
    {
      ...
      buffer.pNext = NULL;
      buffer.flags = 0;
      buffer.size = accessor->size * sizeof(owl_m4);
      buffer.usage =
...

    {
      ...
      buffer.offset = 0;
      buffer.range = accessor->size * sizeof(owl_m4);
...
OWL_INTERNAL enum owl_code owl_scene_load_animations_(struct owl_renderer *r,
  ...
  if (OWL_SCENE_MAX_ANIMATIONS_SIZE <= (int)gltf->animations_size) {
    code = OWL_ERROR_OUT_OF_BOUNDS;
    goto end;
  }

  scene->animations_size = (int)gltf->animations_size;

  for (i = 0; i < (int)gltf->animations_size; ++i) {
    int j;
    ...
    /* verify the samplers size */
    if (OWL_SCENE_ANIMATION_MAX_SAMPLERS_SIZE <=
        (int)from_animation->samplers_size) {
      code = OWL_ERROR_OUT_OF_BOUNDS;
      goto end;
    }

    animation_data->samplers_size = (int)from_animation->samplers_size;
    ...
    /* init the samplers */
    for (j = 0; j < (int)gltf->samplers_size; ++j) {
      int k;
      void const *data;
      ...
      sampler->slot = scene->animation_samplers_size++;

      /* verify the sampler size in the scene */
      if (OWL_SCENE_MAX_SAMPLERS_SIZE <= sampler->slot) {
        code = OWL_ERROR_OUT_OF_BOUNDS;
        goto end;
      }
      ...
      if (OWL_SCENE_ANIMATION_SAMPLER_MAX_INPUTS_SIZE <=
          (int)accessor->size) {
        code = OWL_ERROR_OUT_OF_BOUNDS;
        goto end;
      }

      sampler_data->inputs_size = (int)accessor->size;

      view = accessor->buffer_view;
      data = &((owl_byte const *)
                   view->buffer->data)[accessor->offset + view->offset];

      for (k = 0; k < (int)accessor->size; ++k)
        sampler_data->inputs[k] = ((float const *)data)[k];

      /* set the animation data min and max */
      for (k = 0; k < sampler_data->inputs_size; ++k) {
      ...
      if (OWL_SCENE_ANIMATION_SAMPLER_MAX_OUTPUTS_SIZE <=
          (int)accessor->size) {
        code = OWL_ERROR_OUT_OF_BOUNDS;
        goto end;
      }
    ...
    if (OWL_SCENE_ANIMATION_MAX_CHANNELS_SIZE <= 
        (int)from_animation->channels_size) {
      code = OWL_ERROR_UNKNOWN;
      goto end;
    }

    animation_data->channels_size = (int)from_animation->channels_size;

    for (j = 0; j < (int)from_animation->channels_size; ++j) {
      ...
      channel->slot = scene->animation_channels_size++;

      if (OWL_SCENE_MAX_CHANNELS_SIZE <= channel->slot){
        code = OWL_ERROR_UNKNOWN;
        goto end;
      }
...
enum owl_code owl_scene_init(struct owl_renderer *r, char const *path,
  ...
  scene->materials_size = 0;
  scene->meshes_size = 0;
  scene->primitives_size = 0;
  scene->skins_size = 0;
  scene->animation_samplers_size = 0;
  scene->animation_channels_size = 0;
  scene->animations_size = 0;
