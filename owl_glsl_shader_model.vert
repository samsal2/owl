...
void main() 
{
  ...
  if (0 != node.joint_matrices_count) { 
  	...
  	local_position = ubo.model * node.matrix * skin_matrix * vec4(in_position, 1.0);
    out_normal = normalize(transpose(inverse(mat3(ubo.view * ubo.model * node.matrix * skin_matrix))) * in_normal);
  } else {
  	local_position = ubo.model * node.matrix * vec4(in_position, 1.0);
    out_normal = normalize(transpose(inverse(mat3(ubo.view * ubo.model * node.matrix))) * in_normal);
  }
