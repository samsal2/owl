...
vec3 getIBLContribution(PBRInfo pbrInputs, vec3 n, vec3 reflection) {
  ...
  vec3 brdf = (texture(
                   sampler2D(samplerBRDFLUT, sampler0),
  ...
  vec3 specularLight = SRGBtoLINEAR(tonemap(textureLod(
                                        samplerCube(prefilteredMap, sampler0),
                                        reflection, lod)))
                           .rgb;
...
void main() {
  ...
  if (ubo.debugViewInputs > 0.0) {
    ...
    case 1:
      outColor.rgba = material.baseColorTextureSet > -1
                          ? texture(sampler2D(colorMap, sampler0),
                                    material.baseColorTextureSet == 0 ? inUV0
                                                                      : inUV1)
                          : vec4(1.0f);
      break;
    case 2:
      outColor.rgb = (material.normalTextureSet > -1)
                         ? texture(sampler2D(normalMap, sampler0),
                                   material.normalTextureSet == 0 ? inUV0
                                                                  : inUV1)
                               .rgb
                         : normalize(inNormal);
      break;
    case 3:
      ...
      break;
    case 4:
      outColor.rgb = (material.emissiveTextureSet > -1)
                         ? texture(sampler2D(emissiveMap, sampler0),
                                   material.emissiveTextureSet == 0 ? inUV0
                                                                    : inUV1)
                               .rgb
                         : vec3(0.0f);
      break;
