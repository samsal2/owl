...
vec3 getNormal()
{
	...
	if (0 == material.normal_uv_set)
		tangent_normal =
			texture(sampler2D(normal_map, sampler0), in_uv0).xyz *
				2.0 -
			1;
	else
		tangent_normal =
			texture(sampler2D(normal_map, sampler0), in_uv1).xyz *
				2.0 -
			1;
...
vec3 specularReflection(pbr_info pbr_inputs)
{
	return pbr_inputs.reflectance0 +
	       (pbr_inputs.reflectance90 - pbr_inputs.reflectance0) *
		       pow(clamp(1.0 - pbr_inputs.view_dot_half, 0.0, 1.0),
			   5.0);
}
...
float geometricOcclusion(pbr_info pbr_inputs)
{
	...
	float attenuation_light =
		...
		(normal_dot_light +
		 sqrt(r * r +
		      (1.0 - r * r) * (normal_dot_light * normal_dot_light)));
	float attenuation_view =
		2.0 * normal_dot_view /
		(normal_dot_view +
		 sqrt(r * r +
		      (1.0 - r * r) * (normal_dot_view * normal_dot_view)));
...
void main()
{
	...
	if (material.alpha_mask == 1.0F) {
		if (0 == material.base_color_uv_set) {
			base_color = SRGBtoLINEAR(texture(sampler2D(color_map,
								    sampler0),
							  in_uv0)) *
	...
	if (PBR_WORKFLOW_METALLIC_ROUGHNESS == material.workflow) {
		...
		if (0 == material.physical_descriptor_uv_set) {
			vec4 sample_metallic_roughness = texture(
				sampler2D(physical_descriptor_map, sampler0),
				in_uv0);
			...
			perceptual_roughness = clamp(perceptual_roughness,
						     MINIMUM_ROUGHNESS, 1.0);
	...
	if (PBR_WORKFLOW_SPECULAR_GLOSINESS == material.workflow) {
		if (0 == material.physical_descriptor_uv_set) {
			perceptual_roughness =
				1.0 - texture(sampler2D(physical_descriptor_map,
							sampler0),
					      in_uv0)
					      .a;
		} else if (1 == material.physical_descriptor_uv_set) {
		...
		vec3 specular =
			SRGBtoLINEAR(texture(sampler2D(physical_descriptor_map,
						       sampler0),
					     in_uv0))
				.rgb;

		const float max_specular =
		...
		base_color =
			vec4(mix(base_color_diffuse_part,
				 base_color_specular_part, metallic * metallic),
			     diffuse.a);
	}
	...
	vec3 n = (material.normal_uv_set > -1) ? getNormal() :
						 normalize(in_normal);
	...
	pbr_info inputs = pbr_info(normal_dot_light, normal_dot_view,
				   normal_dot_half, light_dot_half,
				   view_dot_half, perceptual_roughness,
				   metallic, specular_environment_r0,
				   specular_environment_r90, alpha_roughness,
