...
void main()
{
    ...
    if (material.workflow == PBR_WORKFLOW_SPECULAR_GLOSINESS) {
        ...
        if (material.physicalDescriptorTextureSet > -1) {
            perceptualRoughness =
                1.0 - texture(sampler2D(physicalDescriptorMap, sampler0),
        ...
        vec4 diffuse =
            SRGBtoLINEAR(texture(sampler2D(colorMap, sampler0), inUV0));
        vec3 specular =
            SRGBtoLINEAR(
                texture(sampler2D(physicalDescriptorMap, sampler0), inUV0))
                .rgb;
        ...
        baseColor = vec4(mix(baseColorDiffusePart, baseColorSpecularPart,
                             metallic * metallic),
                         diffuse.a);
    }
