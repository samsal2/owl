...
{
	float lod =
		(pbrInputs.perceptualRoughness * ubo.prefilteredCubeMipLevels);
	// retrieve a scale and bias to F0. See [1], Figure 3
	vec3 brdf = (texture(sampler2D(samplerBRDFLUT, sampler0),
			     vec2(pbrInputs.NdotV,
				  1.0 - pbrInputs.perceptualRoughness)))
			    .rgb;
	vec3 diffuseLight =
		SRGBtoLINEAR(
			tonemap(texture(
				samplerCube(samplerIrradiance, sampler0), n)))
			.rgb;

	vec3 specularLight =
		SRGBtoLINEAR(tonemap(textureLod(samplerCube(prefilteredMap,
							    sampler0),
						reflection, lod)))
			.rgb;
...
void main()
{
	...
	if (material.alphaMask == 1.0f) {
		if (material.baseColorTextureSet > -1) {
			baseColor = SRGBtoLINEAR(texture(
					    ...
					    material.baseColorTextureSet == 0 ?
						    inUV0 :
						    inUV1)) *
	...
	if (material.workflow == PBR_WORKFLOW_METALLIC_ROUGHNESS) {
		...
		if (material.physicalDescriptorTextureSet > -1) {
			...
			vec4 mrSample = texture(
				sampler2D(physicalDescriptorMap, sampler0),
				material.physicalDescriptorTextureSet == 0 ?
					inUV0 :
					inUV1);
	...
	if (material.workflow == PBR_WORKFLOW_SPECULAR_GLOSINESS) {
		...
		if (material.physicalDescriptorTextureSet > -1) {
			perceptualRoughness =
				1.0 -
				texture(sampler2D(physicalDescriptorMap,
						  sampler0),
					material.physicalDescriptorTextureSet ==
							0 ?
						inUV0 :
						inUV1)
					.a;
		...
		vec4 diffuse = SRGBtoLINEAR(
			texture(sampler2D(colorMap, sampler0), inUV0));
		vec3 specular =
			SRGBtoLINEAR(texture(sampler2D(physicalDescriptorMap,
						       sampler0),
					     inUV0))
				.rgb;
		...
		baseColor =
			vec4(mix(baseColorDiffusePart, baseColorSpecularPart,
				 metallic * metallic),
			     diffuse.a);
	}
	...
	vec3 l = normalize(
		ubo.lightDir.xyz); // Vector from surface point to light
	...
	if (material.occlusionTextureSet > -1) {
		float ao = texture(sampler2D(aoMap, sampler0),
				   (material.occlusionTextureSet == 0 ? inUV0 :
									inUV1))
				   .r;
	...
	if (ubo.debugViewInputs > 0.0) {
		...
		case 4:
			...
			break;
		case 5:
			outColor.rgb = texture(sampler2D(physicalDescriptorMap,
							 sampler0),
					       inUV0)
					       .bbb;
			break;
